...
public class MasterAudioClipManager : EditorWindow {
		...
		get {
			if (filterClips == null) {
				...
				filterClips.RemoveAll(delegate(AudioInfo obj) {
					return !obj._name.ToLower().Contains(clipList.searchFilter.ToLower());
				});
	...
	private void DisplayClips() {
		...
		scrollPos = GUI.BeginScrollView(new Rect(0, 89, 896, 500), scrollPos, new Rect (0, 89, 880, 25 * FilteredClips.Count + 4));	 
	...
	private void BuildCache() {
		...
		for (var i = 0; i < filePaths.Length; i++) {
			...
			var asset = (AudioClip) AssetDatabase.LoadAssetAtPath(aPath, typeof(AudioClip));
			...
			var newClip = new AudioInfo(aPath, asset.name, importer.threeD, bitrate, importer.forceToMono, importer.format, importer.loadType);
